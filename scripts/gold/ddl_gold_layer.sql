...
SELECT ROW_NUMBER() OVER(
  ...
  CASE
    WHEN ci.cst_gndr != 'n/a' THEN ci.cst_gndr -- created from the master table.
...
-- Creating join for the prd_info and erp_prd_a12 table
...
FROM silver.crm_prd_info cp
  LEFT JOIN silver.erp_px_cat_g1v2 pc ON pc.id = cp.cat_id
WHERE prd_end_dt IS NULL

-- Creating join for sales table with customer and product -- keys
